...
module data_memory (
  ...
  initial begin
    $readmemh("data_memory.mem", rom_memory, DATA_MEMORY_ADDRESS_WIDTH'('h0),
              DATA_MEMORY_ADDRESS_WIDTH'('hFF));  // load ROM memory


    for (i = 0; i < DATA_MEMORY_RAM_DEPTH; i = i + 1)
    ram_memory[i] = RISC_V_DATA_WIDTH'('b0);  // initialize RAM
  end


  always @(*) begin
    //TODO: implement reading from ROM and RAM
    // ???
  ...
  always @(posedge clk or posedge rst) begin
    if (rst) begin
      //TODO: set all RAM entries to zero  
      // ???
      ...
      //TODO: write data to RAM if write signal is set and address is a valid RAM address
      // ???

      if (ctrl_mem_w && (address >= DATA_MEMORY_ROM_DEPTH) && (address < DATA_MEMORY_ROM_DEPTH + DATA_MEMORY_RAM_DEPTH)) begin
