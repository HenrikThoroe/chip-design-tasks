...
module test_prog_tb;
  ...
  initial begin
    $dumpfile("dump.vcd");
    $dumpvars(0, test_prog_tb);
    

    $monitor("Time=%0t | PC=%h | Inst=%h | ALU_Op=%b | ALU_Out=%h | R_Data0=%h | R_Data1=%h | Mem_R=%h", 
             $time, instruction_address, instruction, ctrl_ALU_op, ALU_data_out, reg_read_data_0, reg_read_data_1, mem_read_data);
